...
class myPDF extends PDF_Unicode
{
	...
	function Header()
	{
		...
		$this->SetFontSize(14); 
		$this->Cell(0,8,$title,"T",1,'C');
		$this->SetFontSize(12);
		$this->Cell(0,8,$dept_name."(" . $orastatus_name.")" .$organize_name,0,1,"L");
		$this->Cell(0,8,"列表日期：" . date("Y/m/d") . "  招生人數 : $enrollperson 人  報名人數 : $allperson 人   #表示不合格","B",1,"L");
		$this->Cell(0,5,"",0,1,'C');
		$this->SetFont('font1','B',10);
		$width = ceil(190 / (sizeof($subject_rows['NAME']) +2));
		$max =0;
		//找出科目名稱最長者,以決定欄高
		...
		$this->SetXY($CurrentX+$width*$j,$CurrentY);
		$this->Cell($width+3,$height,"總分(加權後)",1,0,'C');
		$this->Cell($width,$height,"名次",1,1,'C');	
	}	
}
...
$title = "國立彰化師範大學" . $title_rows['TITLE_NAME'][0] ."成績總表";
...
for($i=0;$i<sizeof($ora_rows['ID']);$i++)
{
	...
	$sql_per="select student_id,decode(allnumber,0,999,allnumber) allnumber,allscore,allratescore $sql_persub
		from person a
	...
	{   
		...
		$pdf->Cell($width+3,7,$allratescore,1,0,"L");
		$pdf->Cell($width,7,$allnumber,1,1,"L");
	}
}
$pdf->Cell(0,7,"共有：" . $allperson ." 人",0,0,"L");
